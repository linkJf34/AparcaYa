...
public interface RegistroEntradaSalidaRepository extends JpaRepository<RegistroEntradaSalida, Long> {
    ...
    @Query("SELECT COALESCE(SUM(r.precio), 0) FROM RegistroEntradaSalida r " +
            "WHERE r.sede = :sede AND r.estado = 'COBRADO' " +
            "AND r.fechaHoraEntrada >= :inicio AND r.fechaHoraEntrada < :fin")
